...
extension Shape {
    ...
    public func stripe(_ fgcolor: Color, width: CGFloat = 1.0, ratio: CGFloat = 5.0) -> some View {
        ...
        return ZStack {
            self.fill(ImagePaint(image: Image(decorative: stripePattern, scale: 1))).opacity(0.4)
            self.stroke()
        }
    }
}
